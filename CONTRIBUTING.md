...
## Contributing to the Shadow CLI

Thanks for your interest in improving Shadow CLI!
...
The Shadow CLI project adheres to the [Rust Code of Conduct][rust-coc]. This code of conduct describes the _minimum_ behavior
...
1. **By opening an issue:** For example, if you believe that you have uncovered a bug
   in Shadow CLI, creating a new issue in the issue tracker is the way to report it.
...
- The Shadow CLI version you are on (and that it is up to date)
...
Pull requests are the way concrete changes are made to the code, documentation, and dependencies of Shadow CLI.
...
If the change being proposed alters code, it is either adding new functionality to Shadow CLI, or fixing existing, broken
functionality.
In both of these cases, the pull request should include one or more tests to ensure that Shadow CLI does not regress in the
future.
...
workspace (`cargo test -p shadow-cli mytest`).
...
1. Does this change make sense for Shadow CLI?
2. Does this change make Shadow CLI better, even if only incrementally?
...
can typically be fixed by the Shadow CLI maintainers merging the pull request, but they can also be an opportunity for the
...
of the pull request. Yes, we may merge a particular change that makes Shadow CLI better, but the individual might just not
want to have anything to do with Shadow CLI ever again. The goal is not just having good code.
